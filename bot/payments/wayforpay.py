...
from services import activate_or_extend, _tz_aware_utc
    ...
    try:
        ...
        async with Session() as s:
            ...
            if sub and sub.updated_at and _tz_aware_utc(sub.updated_at) > order_dt:
                print("⛔ Stale callback ignored (after unsubscribe):", order_ref)
                return
        ...
        async with Session() as s:
            if pay:
                ...
                s.add(pay)
            await s.commit()
            print(f"💰 Payment recorded: user={user_id}, order_ref={order_ref}")

        # Продлеваем или активируем подписку
        await activate_or_extend(bot, user_id)
        print(f"✅ Subscription activated/extended for user {user_id}")
